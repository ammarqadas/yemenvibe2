...
class ItemsController extends VibeController
{
    ...
    {		$itemq=Items::find();	
	    	...
	    	$items = $itemq->select(['id','feedId','title','url','read','itemDate','slug','thumb'])->with(
							[
								'feed' => function ($query) {
	        ...
	        $headers= Items::find()->select(['id','feedId','title','url','read','itemDate','enclosureUrl','content','slug','thumb'])->with(
							[
								'feed' => function ($query) {
