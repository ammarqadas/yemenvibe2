...
use common\models\SitemapEntityInterface;
...
class Sitemap extends Component
{
	...
	public $sitemapSubDir="sitemap";
	public $sitemapIndex="sitemap.xml";
    ...
    {
        $this->path = "{$this->sitemapDirectory}/_{$this->sitemapIndex}";
        ...
        if (isset(\Yii::$app->urlManager->baseUrl)) {
            $baseUrl =\Yii::$app->urlManager->baseUrl.'/'.$this->sitemapSubDir;
        }
        ...
        fclose($this->handle);
        $this->gzipFile();
		$f="{$this->sitemapDirectory}/{$this->sitemapIndex}";
		 if(file_exists($f))unlink($f);
		 rename("{$this->sitemapDirectory}/_{$this->sitemapIndex}",$f);
    }
    ...
    protected function updateSitemaps()
    {
		...
		$path="{$this->sitemapDirectory}/{$this->sitemapSubDir}";
	    foreach (glob("{$path}/sitemap*.xml*") as $filePath) {
            unlink($filePath);
        }
        // rename new files (without '_')
        foreach (glob("{$path}/_sitemap*.xml*") as $filePath) {
	...
	$this->path ="{$this->sitemapDirectory}/{$this->sitemapSubDir}/_{$fileName}";
